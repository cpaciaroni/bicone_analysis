...
def GetEndpoints(z,files=True, plots=False):
    head, ec = endpoints.getEndpointWave(z, dirc='/Users/Caroline/Desktop/bicones/Mag_0A_Caroline/')
    ...
    if plots:
        ...
        pyp.semilogy(ec.fd.freq.values, abs(ec.fd.Ex)*dt, 'r', linewidth=2, alpha=0.5)
        pyp.semilogy(ec.fd.freq.values, abs(ec.fd.Ey)*dt, 'b', linewidth=2, alpha=0.5)
        pyp.semilogy(ec.fd.freq.values, abs(ec.fd.Ez)*dt, 'g', linewidth=2, alpha=0.5)
        #pyp.semilogy(fft.fftshift(freq), abs(fft.fftshift(ffth))*dt2, 'b--', linewidth=4, alpha=0.75)
        #pyp.semilogy(fft.fftshift(freq), abs(fft.fftshift(fftv))*dt2, 'r--', linewidth=4, alpha=0.75)
        ...
        pyp.figure(2)
        ax = pyp.subplot()
        pyp.plot(ec.wave.time.values, ec.wave.Ex, 'r', linewidth=2, alpha=0.5,label='$E_x$')
        pyp.plot(ec.wave.time.values, ec.wave.Ey, 'b', linewidth=2, alpha=0.5,label='$E_y$')
        pyp.plot(ec.wave.time.values, ec.wave.Ez, 'g', linewidth=2, alpha=0.5,label='$E_z$')
        #pyp.plot(times2, eh2, 'b--', linewidth=4, alpha=0.75) #downsampled
        #pyp.plot(times2, ev2, 'r--', linewidth=2, alpha=0.75)
        pyp.title('E-field components with B-field off (I = 0A)',fontsize=14)
        pyp.xlabel("Time (s)",fontsize = 14)
        pyp.ylabel("Electric Field (V/m)",fontsize = 14)
        pyp.legend(loc="lower left",fontsize=14)
        ax.tick_params(axis='both', which='major', labelsize=14,width=2)
        ax.tick_params(axis='both', which='minor', labelsize=12,width=2)
        pyp.xlim(4.8e-8, 5.1e-8)
        ax.spines['top'].set_linewidth(1.5)
        ax.spines['right'].set_linewidth(1.5)
        ax.spines['bottom'].set_linewidth(1.5)
        ax.spines['left'].set_linewidth(1.5)
        
        pyp.savefig('/Users/Caroline/Desktop/Bicones/E_comps_B_off.png',format='png',dpi=500)
                
        
        pyp.figure(3)
        ax = pyp.subplot()
        pyp.plot(ec.wave.time.values, ec.wave.Eh, 'r', linewidth=2, alpha=0.5, label = '$E_H$')
        pyp.plot(ec.wave.time.values, ec.wave.Ev, 'b', linewidth=2, alpha=0.5, label = '$E_V$')
        #pyp.plot(times2, eh2, 'b--', linewidth=4, alpha=0.75)
        #pyp.plot(times2, ev2, 'r--', linewidth=2, alpha=0.75)
        pyp.title('Simulated E-fields with B-field on (I = 2400A)',fontsize=14)
        pyp.xlabel("Time (s)",fontsize = 14)
        pyp.ylabel("Electric Field (V/m)",fontsize = 14)
        pyp.legend(loc="upper left",fontsize=14)
        ax.tick_params(axis='both', which='major', labelsize=14,width=2)
        ax.tick_params(axis='both', which='minor', labelsize=12,width=2)
        pyp.xlim(4.8e-8, 5.1e-8)
        ax.spines['top'].set_linewidth(1.5)
        ...
        #pyp.savefig('/Users/Caroline/Desktop/Bicones/E_Sims_B_off.png',format='png',dpi=500)
        pyp.show()
